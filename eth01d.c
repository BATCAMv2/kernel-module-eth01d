...
static int eth01d_update_measurements(struct device *dev)
{
     ...
     */
	if (time_after(jiffies, eth01d->last_update + (8 * HZ)) ||
	    !eth01d->valid) {
		/*
...
/* sysfs attributes */
static SENSOR_DEVICE_ATTR(temp1_input, S_IRUGO, eth01d_temperature_show, NULL,
			  0);
static SENSOR_DEVICE_ATTR(humidity1_input, S_IRUGO, eth01d_humidity_show, NULL,
			  0);
			...
			const struct i2c_device_id *id)
{
	...
	hwmon_dev = devm_hwmon_device_register_with_groups(
		dev, client->name, eth01d, eth01d_groups);
	return PTR_ERR_OR_ZERO(hwmon_dev);
}

/* Device ID table */
static const struct i2c_device_id eth01d_id[] = { { "eth01d", 0 }, {} };
